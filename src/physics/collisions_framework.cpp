...
{
    float min = glm::dot(axis, bb._vertices[0] + bb.motionMaster()->position2D());
    float max = min;
    for (int i = 1; i < bb._vertices.size(); ++i)
    {
        float tmp = glm::dot(axis, bb._vertices[i] + bb.motionMaster()->position2D());
        if (tmp < min)
        {
            min = tmp;
        }
...
glm::vec2 CollisionsFramework::project(const CircularBoundingBox& bb, glm::vec2 axis)
{
    ...
    auto p0 = bb.center2D() + bb.motionMaster()->position2D() - axis * bb.radius();
    auto p1 = bb.center2D() + bb.motionMaster()->position2D() + axis * bb.radius();
